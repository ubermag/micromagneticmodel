...
def unittest(c):
    ...
    result = micromagneticmodel.test()
...
def coverage(c):
    ...
    result = pytest.main(["-v", "--cov", "micromagneticmodel", "--cov-report", "xml"])
    raise Exit(code=result)
...
def docs(c):
    """Run doctests."""
    result = pytest.main(
        ["-v", "--doctest-modules", "--ignore", "micromagneticmodel/tests", "micromagneticmodel"]
    )
    raise Exit(code=result)
